...
void live()
{
  ...
  if (analogRead(NOTE_IN_PIN) > 20) { // "add > 20" to remove noise
    // output sound using the button-resistor ladder combo and the photo cell
    tone(BUZZER_PIN, (analogRead(NOTE_IN_PIN) + (offsetFrequency * 10)), duration / 2);
  }
}
...
void startUpTimer()
{
  //initialize the starTime variable when button is pressed
  startTime = millis();
  timePassed = 0;
...
{
  offsetFrequency = getPhotoFrequency();
  // output sound using buttons and photo cell
...
{
  //find average of photocell value in record mode
  int rawSenseVal = offsetFrequency;
  ...
  delay(100);
  //return average value of photo cell
  return averageVal;
}
/******************COLORLED(): IMPLEMENT *********************************
   INSTRUCTIONS:
...
void playWithDuration()
{
  for (int i = 0; i < MAX_NOTES; i++)
  {
    ...
    //duration array AND delay
    delay(durations[i]);
    delay(duration / 2);
    colorLED(durations[i] / 5);
    //if button is pressed, play tone
    if (digitalRead(BUTTON_MODE_PIN) == HIGH)
      //exit the for loop
      break;
  }
